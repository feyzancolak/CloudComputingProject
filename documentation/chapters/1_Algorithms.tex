...
\begin{algorithm}
    ...
    \Procedure{map}{$key, value, context$}
        ...
        \For{each character $ch$ in $line$}
            \If{$ch$ matches $\text{characterPattern}$}
    ...
    \Procedure{cleanup}{$context$}
        \For{each entry in $\text{characterCounts}$}
...
\Class{Mapper}{}
    ...
    \State $H \gets$ new AssociativeArray \Comment{Initialize map for character counts}
