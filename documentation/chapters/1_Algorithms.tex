...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \Method{reduce}{character $c$, count[] $values$}
...
\begin{algorithm}
    ...
    \Class{Mapper}{}
        ...
        \For{$\text{each character } c \text{ in } line$}
            \State $\text{sum} \gets \text{sum + 1}$
        \EndFor

        \State{$\text{map.add(c, sum)}$}
        \EndMethod
        \\
    
        \Method{Close}{}
        \For{$\text{each entry } (character, count) \text{ in } map$}
    ...
    \Class{Reducer}{}
        ...
        \Method{Reduce}{\text{character $c$, count[] $values$}}
        ...
        \EndFor
        \State $\text{Emit(c, sum)}$ \Comment{Output final count}
        \EndMethod
    \EndClass
    \end{algorithmic}
